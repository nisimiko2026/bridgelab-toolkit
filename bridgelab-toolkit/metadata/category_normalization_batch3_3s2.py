"""Safe Phase 3A Batch 3.3s2 entry-management normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S2 = {
    "play/declarer-play/planning/entry-management.md": (
        "declarer-play",
        [
            "declarer play",
            "declarer-play",
...
def build_category_normalization_batch3_3s2_report(
    root: Path,
    ...
    for article, (required_subcategory, required_tags) in sorted(
        REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S2.items()
    ):
    ...
    return CategoryNormalizationReport(
        len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S2), tuple(actions)
    )


def apply_category_normalization_batch3_3s2_report(
