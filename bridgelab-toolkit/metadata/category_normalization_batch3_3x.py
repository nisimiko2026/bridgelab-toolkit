"""Safe Phase 3A Batch 3.3x Slam Bidding category normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3X = {
    "bidding/conventions/slam-conventions/slam-trial-bids.md": [
        "blackwood",
        "conventions",
        "jacoby",
        "roman keycard",
...
OBSERVED_CATEGORY = "Slam Bidding"
...
def build_category_normalization_batch3_3x_report(
    root: Path,
    ...
    reviewed_family = set(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3X)
    if observed_family not in (reviewed_family, set()):
        ...
        raise RuntimeError(
            f"Reviewed Slam Bidding family completeness mismatch: missing={missing!r}, extra={extra!r}"
        )

    actions: list[CategoryNormalizationAction] = []
    for article, required_tags in REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3X.items():
        ...
        if (
            tags != required_tags
            or "slam bidding" not in required_tags
            or "conventions" not in required_tags
            or PROPOSED_CATEGORY in required_tags
        ):
            raise RuntimeError(
                f"Reviewed frozen-tag precondition mismatch: {article}: expected exact retained tags {required_tags!r} with semantic tags retained and no broad 'bidding' tag, observed {tags!r}"
            )
    ...
    return CategoryNormalizationReport(1, tuple(actions))


def apply_category_normalization_batch3_3x_report(
