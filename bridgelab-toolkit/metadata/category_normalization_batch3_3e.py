"""Safe Phase 3A Batch 3.3e probability category normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3E = {
    "play/declarer-play/probability/a-priori-and-a-posteriori-probabilities.md",
    "play/declarer-play/probability/combination-counts.md",
    "play/declarer-play/probability/conditional-probability.md",
    "play/declarer-play/probability/missing-honor-probabilities.md",
    "play/declarer-play/probability/probability-in-bridge.md",
    "play/declarer-play/probability/suit-distributions.md",
}
OBSERVED_CATEGORY = "probability"
PROPOSED_CATEGORY = "play"
REQUIRED_SUBCATEGORY = "declarer-play"
...
def build_category_normalization_batch3_3e_report(root: Path) -> CategoryNormalizationReport:
    """Build the exact reviewed six-file batch entirely in memory."""

    root = root.resolve()
    actions: list[CategoryNormalizationAction] = []
    for article in sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3E):
        ...
        tags = data.get("tags")
        if not isinstance(tags, list) or PROPOSED_CATEGORY in tags:
            raise RuntimeError(
                f"Reviewed frozen-tag precondition mismatch: {article}: expected "
                "an existing tag list with no broad 'play' tag"
            )
    ...
    return CategoryNormalizationReport(
        selected_files=len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3E),
        actions=tuple(actions),
    )


def apply_category_normalization_batch3_3e_report(
