"""Safe Phase 3A Batch 3.3c defensive-method category normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3C = {
    "bidding/conventions/defensive-methods/crash.md",
    "bidding/conventions/defensive-methods/suction.md",
    "bidding/conventions/defensive-methods/twerb.md",
}
OBSERVED_CATEGORY = "Convention"
PROPOSED_CATEGORY = "bidding"
REQUIRED_SUBCATEGORY = "Defenses"
...
def build_category_normalization_batch3_3c_report(root: Path) -> CategoryNormalizationReport:
    """Build the exact reviewed three-file batch entirely in memory."""

    root = root.resolve()
    actions: list[CategoryNormalizationAction] = []
    for article in sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3C):
    ...
    return CategoryNormalizationReport(
        selected_files=len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3C),
        actions=tuple(actions),
    )


def apply_category_normalization_batch3_3c_report(
