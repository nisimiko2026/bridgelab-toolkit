"""Safe Phase 3A Batch 3.3w planning category normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3W = {
    "play/declarer-play/planning/choosing-a-line.md": [
        "declarer-play",
        "discard",
        "double",
        "endplay",
    ...
    "play/declarer-play/planning/creating-extra-tricks.md": [
        "declarer-play",
        "discard",
        "endplay",
        "finesse",
    ...
    "play/declarer-play/planning/visualization.md": [
        "declarer-play",
        "double",
        "endplay",
        "lead",
...
def build_category_normalization_batch3_3w_report(
    root: Path,
    ...
    reviewed_family = set(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3W)
    if observed_family not in (reviewed_family, set()):
        ...
        raise RuntimeError(
            f"Reviewed planning family completeness mismatch: missing={missing!r}, extra={extra!r}"
        )

    actions: list[CategoryNormalizationAction] = []
    for article, required_tags in sorted(
        REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3W.items()
    ):
        ...
        if (
            tags != required_tags
            or OBSERVED_CATEGORY not in required_tags
            or PROPOSED_CATEGORY in required_tags
        ):
            raise RuntimeError(
                f"Reviewed frozen-tag precondition mismatch: {article}: expected exact retained tags {required_tags!r} with 'planning' retained and no broad 'play' tag, observed {tags!r}"
            )
    ...
    return CategoryNormalizationReport(
        len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3W), tuple(actions)
    )


def apply_category_normalization_batch3_3w_report(
