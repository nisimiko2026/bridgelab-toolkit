"""Safe Phase 3A Batch 3.3h declarer-play category normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3H = {
    "play/declarer-play/general-techniques/establishing-suits.md",
}
...
def build_category_normalization_batch3_3h_report(root: Path) -> CategoryNormalizationReport:
    """Build the exact reviewed one-file batch entirely in memory."""

    root = root.resolve()
    actions: list[CategoryNormalizationAction] = []
    for article in sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3H):
        ...
        if tags != REQUIRED_TAGS or PROPOSED_CATEGORY in tags:
            raise RuntimeError(
                ...
                f"exact retained tags {REQUIRED_TAGS!r} and no broad 'play' tag, "
                f"observed {tags!r}"
            )
    ...
    return CategoryNormalizationReport(
        selected_files=len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3H),
        actions=tuple(actions),
    )


def apply_category_normalization_batch3_3h_report(
