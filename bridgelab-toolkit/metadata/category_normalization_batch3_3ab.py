"""Safe Phase 3A Batch 3.3ab Partnership Agreements category normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3AB = {
    "bidding/principles/partnership/convention-cards.md": [
        "blackwood", "cappelletti", "competitive", "cue bid", "dont", "double",
        "drury", "forcing", "gerber", "jacoby", "lebensohl", "multi", "negative",
        "notrump", "opening", "partnership agreements", "principles", "puppet stayman",
        "slam", "stayman", "support", "takeout", "walsh",
    ],
    "bidding/principles/partnership/disclosure.md": [
        "discard", "forcing", "lebensohl", "notrump", "opening",
        "partnership agreements", "principles", "response", "transfer",
    ],
}
OBSERVED_CATEGORY = "Partnership Agreements"
...
def build_category_normalization_batch3_3ab_report(root: Path) -> CategoryNormalizationReport:
    """Build the exact reviewed two-file family entirely in memory."""
    root = root.resolve()
    ...
    reviewed_family = set(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3AB)
    if observed_family not in (reviewed_family, set()):
        ...
        raise RuntimeError(
            "Reviewed Partnership Agreements family completeness mismatch: "
            f"missing={missing!r}, extra={extra!r}"
        )

    actions = []
    for article, required_tags in sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3AB.items()):
        path = root / article
        if not path.is_file():
        ...
        if (
            tags != required_tags
            or "partnership agreements" not in required_tags
...
def apply_category_normalization_batch3_3ab_report(
