"""Safe Phase 3A Batch 3.3s1 trump-control normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S1 = {
    "play/declarer-play/trump-play/trump-control.md": (
        "declarer-play",
        ["declarer play", "declarer-play", "opening", "ruff", "support"],
    ),
}
OBSERVED_CATEGORY = "Declarer Play"
...
def build_category_normalization_batch3_3s1_report(
    root: Path,
) -> CategoryNormalizationReport:
    """Build the exact reviewed one-file batch entirely in memory."""
    root = root.resolve()
    actions: list[CategoryNormalizationAction] = []
    for article, (required_subcategory, required_tags) in sorted(
        REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S1.items()
    ):
        ...
        if (
            tags != required_tags
            or "declarer play" not in required_tags
            or "declarer-play" not in required_tags
    ...
    return CategoryNormalizationReport(
        len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S1), tuple(actions)
    )


def apply_category_normalization_batch3_3s1_report(
