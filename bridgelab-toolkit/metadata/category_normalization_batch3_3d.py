"""Safe Phase 3A Batch 3.3d defensive-play category normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3D = {
    "play/defence/endgame-defence/avoiding-throw-in.md",
    "play/defence/endgame-defence/preventing-endplay.md",
}
OBSERVED_CATEGORY = "Defensive Play"
PROPOSED_CATEGORY = "play"
REQUIRED_SUBCATEGORY = "Defensive Techniques"
REQUIRED_TAG = "defensive play"
...
def build_category_normalization_batch3_3d_report(root: Path) -> CategoryNormalizationReport:
    """Build the exact reviewed two-file batch entirely in memory."""

    root = root.resolve()
    actions: list[CategoryNormalizationAction] = []
    for article in sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3D):
        ...
        tags = data.get("tags")
        if not isinstance(tags, list) or REQUIRED_TAG not in tags or "play" in tags:
            raise RuntimeError(
                f"Reviewed frozen-tag precondition mismatch: {article}: expected "
                f"existing {REQUIRED_TAG!r} tag and no 'play' tag"
            )
    ...
    return CategoryNormalizationReport(
        selected_files=len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3D),
        actions=tuple(actions),
    )


def apply_category_normalization_batch3_3d_report(
