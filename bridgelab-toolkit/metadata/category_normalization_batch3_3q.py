"""Safe Phase 3A Batch 3.3q Systems normalization."""
...
def build_category_normalization_batch3_3q_report(
    root: Path,
    ...
    for article, (required_subcategory, required_tags) in sorted(
        REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3Q.items()
    ):
        ...
        tags = data.get("tags")
        if tags != required_tags or PROPOSED_CATEGORY in required_tags:
            raise RuntimeError(
    ...
    return CategoryNormalizationReport(
        len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3Q), tuple(actions)
    )


def apply_category_normalization_batch3_3q_report(
