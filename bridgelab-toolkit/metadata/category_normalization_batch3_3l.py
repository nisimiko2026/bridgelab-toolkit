"""Safe Phase 3A Batch 3.3l relay Convention normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3L = {
    "bidding/conventions/relay/kokish-relay.md": (
        "Relay Bidding",
        [
            "blackwood",
            "convention",
    ...
    "bidding/conventions/relay/relay-bidding.md": (
        "conventions",
        [
            "blackwood",
            "blue club",
    ...
    "bidding/conventions/relay/relay-principle.md": (
        "Relay Bidding",
        [
            "competitive",
            "convention",
    ...
    "bidding/conventions/relay/spiral-relay.md": (
        "Relay Bidding",
        [
            "convention",
            "kokish",
    ...
    "bidding/conventions/relay/step-responses.md": (
        "Relay Bidding",
        [
            "blackwood",
            "convention",
...
def build_category_normalization_batch3_3l_report(
    root: Path,
) -> CategoryNormalizationReport:
    """Build the exact reviewed five-file batch entirely in memory."""

    root = root.resolve()
    actions: list[CategoryNormalizationAction] = []
    for article, (required_subcategory, required_tags) in sorted(
        REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3L.items()
    ):
        ...
        if data.get("subcategory") != required_subcategory:
            raise RuntimeError(
                ...
                f"expected {required_subcategory!r}, "
    ...
    return CategoryNormalizationReport(
        selected_files=len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3L),
        actions=tuple(actions),
    )


def apply_category_normalization_batch3_3l_report(
