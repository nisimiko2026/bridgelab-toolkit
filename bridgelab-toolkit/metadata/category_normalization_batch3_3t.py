"""Safe Phase 3A Batch 3.3t Polish Club Systems normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3T = {
    "bidding/systems/polish-club.md": (
...
def build_category_normalization_batch3_3t_report(
    root: Path,
    ...
    for article, (required_subcategory, required_tags) in sorted(
        REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3T.items()
    ):
        ...
        if (
            tags != required_tags
            or "ambiguous club systems" not in required_tags
            or "systems" not in required_tags
    ...
    return CategoryNormalizationReport(
        len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3T), tuple(actions)
    )


def apply_category_normalization_batch3_3t_report(
