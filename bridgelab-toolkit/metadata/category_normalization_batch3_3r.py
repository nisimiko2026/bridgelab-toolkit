"""Safe Phase 3A Batch 3.3r Defensive Play normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3R = {
    "play/defence/opening-leads/counting-from-the-opening-lead.md": (
        "Defensive Techniques",
        ["defensive play", "defensive techniques", "lead", "opening"],
    ),
    "play/defence/opening-leads/leads-against-notrump.md": (
        "Opening Leads",
        [
            "defensive play",
            "lead",
    ...
    "play/defence/planning/forcing-the-declarer.md": (
        "Defensive Techniques",
        ["defensive play", "defensive techniques", "forcing", "lead", "ruff"],
    ),
    "play/defence/planning/safe-switches.md": (
        "Defensive Techniques",
        [
            "defensive play",
            "defensive techniques",
    ...
    "play/defence/planning/timing-the-defence.md": (
        "Defensive Techniques",
        [
            "defensive play",
            "defensive techniques",
    ...
    "play/defence/techniques/active-defence.md": (
        "Defensive Techniques",
        [
            "defensive play",
            "defensive techniques",
    ...
    "play/defence/techniques/discard-strategy.md": (
        "Defensive Techniques",
        [
            "defensive play",
            "defensive techniques",
    ...
    "play/defence/techniques/forcing-defence.md": (
        "Defensive Techniques",
        ["defensive play", "defensive techniques", "forcing", "lead", "ruff"],
    ),
    "play/defence/techniques/passive-defence.md": (
        "Defensive Techniques",
        [
            "defensive play",
            "defensive techniques",
...
OBSERVED_CATEGORY = "Defensive Play"
...
def build_category_normalization_batch3_3r_report(
    root: Path,
) -> CategoryNormalizationReport:
    """Build the exact reviewed nine-file batch entirely in memory."""
    root = root.resolve()
    actions: list[CategoryNormalizationAction] = []
    for article, (required_subcategory, required_tags) in sorted(
        REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3R.items()
    ):
        ...
        if (
            tags != required_tags
            or "defensive play" not in required_tags
            or PROPOSED_CATEGORY in required_tags
        ):
            raise RuntimeError(
                f"Reviewed frozen-tag precondition mismatch: {article}: expected exact retained tags {required_tags!r} and no broad 'play' tag, observed {tags!r}"
            )
    ...
    return CategoryNormalizationReport(
        len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3R), tuple(actions)
    )


def apply_category_normalization_batch3_3r_report(
