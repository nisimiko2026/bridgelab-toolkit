"""Safe Phase 3A Batch 3.3ac Principles category normalization."""
...
ARTICLE = "bidding/conventions/game-invitations/game-invitations.md"
REQUIRED_TAGS = [
    "bidding principles", "drury", "jacoby", "opening",
    "principles", "slam", "support", "two over one",
]
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3AC = {ARTICLE: REQUIRED_TAGS}
OBSERVED_CATEGORY = "Principles"
PROPOSED_CATEGORY = "bidding"
REQUIRED_SUBCATEGORY = "Bidding Principles"
...
def build_category_normalization_batch3_3ac_report(root: Path) -> CategoryNormalizationReport:
    ...
    reviewed_family = set(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3AC)
    if observed_family not in (reviewed_family, set()):
        ...
        raise RuntimeError(
            f"Reviewed Principles family completeness mismatch: missing={missing!r}, extra={extra!r}"
        )
    path = root / ARTICLE
    if not path.is_file():
    ...
    if (
        tags != REQUIRED_TAGS
        or "bidding principles" not in REQUIRED_TAGS
...
def apply_category_normalization_batch3_3ac_report(
