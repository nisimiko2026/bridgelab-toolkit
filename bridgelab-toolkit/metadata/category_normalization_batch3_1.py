"""Safe Phase 3A Batch 3.1 bidding-system category normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_1 = {
    "bidding/systems/blue-club.md": "Bidding Systems",
    "bidding/systems/standard-american.md": "Bidding System",
}
...
def build_category_normalization_batch3_1_report(root: Path) -> CategoryNormalizationReport:
    """Build the exact reviewed two-file batch entirely in memory."""

    root = root.resolve()
    actions: list[CategoryNormalizationAction] = []
    for article, expected in sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_1.items()):
    ...
    return CategoryNormalizationReport(
        selected_files=len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_1),
        actions=tuple(actions),
    )


def apply_category_normalization_batch3_1_report(
