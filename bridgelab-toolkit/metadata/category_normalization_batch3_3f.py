"""Safe Phase 3A Batch 3.3f probability category normalization."""
...
REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3F = {
    "play/principles/eight-ever-nine-never.md",
}
OBSERVED_CATEGORY = "probability"
PROPOSED_CATEGORY = "play"
REQUIRED_SUBCATEGORY = "principles"
REQUIRED_TAGS = ["finesse", "principles", "probability"]
...
def build_category_normalization_batch3_3f_report(root: Path) -> CategoryNormalizationReport:
    """Build the exact reviewed one-file batch entirely in memory."""

    root = root.resolve()
    actions: list[CategoryNormalizationAction] = []
    for article in sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3F):
    ...
    return CategoryNormalizationReport(
        selected_files=len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3F),
        actions=tuple(actions),
    )


def apply_category_normalization_batch3_3f_report(
