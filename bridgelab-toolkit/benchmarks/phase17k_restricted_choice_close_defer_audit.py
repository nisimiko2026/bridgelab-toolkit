...
PHASE = "17K"
BASELINE_PHASE = "17J"
CANDIDATE = "Restricted Choice Close/Defer Audit"
...
class Phase17KResult:
    ...
    source_provenance_gap_resolved: bool
    vacant_places_input_contract_gap_resolved: bool
    observation_semantics_gap_resolved: bool
    precision_and_rounding_gap_resolved: bool
    architecture_readiness_gap_resolved: bool
...
def run_audit() -> Phase17KResult:
    ...
    vacant_git_history_found = _git_history_exists("*vacant-places.md")

    # Phase 17K is a conservative close/defer audit. These booleans
    # describe whether the evidence already present closes each blocker.
    # No bridge semantics, formulas, precision defaults, or provenance rules
    # are invented here.
    source_provenance_gap_resolved = False
    vacant_places_input_contract_gap_resolved = False
    observation_semantics_gap_resolved = False
    precision_and_rounding_gap_resolved = (
        explicit_precision_rounding_contract
    )
    architecture_readiness_gap_resolved = (
        restricted_question_exists
        and vacant_question_exists
        and restricted_engine_registered
        and vacant_engine_registered
    )
    ...
    gates = (
        GateResult(
            ...
            detail=(
                "Core Restricted Choice / Vacant Places provenance remains "
                "unresolved; the discovered historical backup is not Git-tracked "
                "and is not identical to the current Vacant Places source."
            ),
        ),
        GateResult(
    ...
    return Phase17KResult(
        ...
        vacant_places_git_history_found=vacant_git_history_found,
        ...
        source_provenance_gap_resolved=source_provenance_gap_resolved,
        vacant_places_input_contract_gap_resolved=(
            vacant_places_input_contract_gap_resolved
        ),
        observation_semantics_gap_resolved=observation_semantics_gap_resolved,
        precision_and_rounding_gap_resolved=(
            precision_and_rounding_gap_resolved
        ),
        architecture_readiness_gap_resolved=(
        ...
        phase17_closed=True,
        resume_condition=(
            "Resume Restricted Choice / Vacant Places production work only "
            "when new authoritative source evidence resolves one or more "
            "of the unresolved production blockers."
        ),
    )
