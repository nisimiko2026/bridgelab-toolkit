...
def _git_status(path: Path, root: Path) -> tuple[bool, str]:
    ...
    status = _run_git("status", "--short", "--", relative)
...
def run_audit() -> Phase17FResult:
    ...
    return Phase17FResult(
        ...
        gates=gates,
        phase17g_direction="RESTRICTED_CHOICE_CONTRACT_ENRICHMENT",
    )
