...
from metadata.category_normalization_batch3_3k import (
    REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3K,
    apply_category_normalization_batch3_3k_report,
    build_category_normalization_batch3_3k_report,
)
from metadata.sentinel_cleanup import _atomic_write


def article_text(
    tags: list[str], newline: str = "\n", category: str = "Convention"
) -> str:
    tag_lines = "".join(f"  - {tag}{newline}" for tag in tags)
    return (
        f"---{newline}title: Competitive Convention{newline}"
        f"description: Exact fixture.{newline}category: {category}{newline}"
        f"subcategory: conventions{newline}difficulty: Intermediate{newline}"
        f"tags:{newline}{tag_lines}systems: []{newline}aliases: []{newline}"
        f"acronyms: []{newline}references:{newline}  - retained/reference{newline}"
        f"last_updated: 2026-08-23{newline}status: Draft{newline}unknown: exact{newline}"
        f"---{newline}# Competitive Convention{newline}{newline}Body unchanged.{newline}"
    )


class CategoryNormalizationBatch33kTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def targets(self) -> dict[str, Path]:
        ...
        for index, (relative, tags) in enumerate(
            sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3K.items())
        ):
    ...
    def invoke(self, *args: str):
        return self.runner.invoke(
            app,
            [
                "repair-category-normalization-batch3-3k",
                "--root",
                str(self.root),
                *args,
            ],
    ...
    def test_exact_selection_excludes_other_and_plural_competitive_files(self) -> None:
        self.assertEqual(len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3K), 10)
        targets = self.targets()
        tags = next(iter(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3K.values()))
        excluded_data = {
            "bidding/conventions/competitive/aspro.md": "Conventions",
            "bidding/conventions/competitive/ghestem.md": "Conventions",
            "bidding/conventions/competitive/good-bad-2nt.md": "Convention Card",
            "bidding/conventions/competitive/unrelated.md": "Convention",
        }
        excluded = []
        ...
        self.assertEqual(first.output.count("SET CATEGORY |"), 10)
        self.assertIn("Files selected      : 10", first.output)
    ...
    def test_category_only_lf_crlf_exact_tags_and_subcategory(self) -> None:
        ...
        report = build_category_normalization_batch3_3k_report(self.root)
        ...
        for action in report.actions:
            ...
            tags = REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3K[action.article]
    ...
    def test_stale_category_subcategory_and_tag_failures(self) -> None:
        ...
        for old, new, message in (
            ...
            with self.assertRaisesRegex(RuntimeError, message):
                build_category_normalization_batch3_3k_report(self.root)
    ...
    def test_path_preserving_backups_and_atomic_failure(self) -> None:
        ...
        with patch(
            "metadata.category_normalization_batch3_3k._atomic_write",
    ...
    def test_idempotence_explicit_root_apply_and_cli_help(self) -> None:
        ...
        self.assertEqual(
            build_category_normalization_batch3_3k_report(self.root).actions, ()
        )
        ...
        help_result = self.runner.invoke(
            app, ["repair-category-normalization-batch3-3k", "--help"]
        )
