...
from benchmarks.phase17g_restricted_choice_supporting_source_audit import (
...
def _recorded():
    validation = validate_historical_report("17G")
    assert validation.is_valid_recorded_output
    assert validation.payload is not None
    return validation.payload


def test_phase17g_candidate_and_classification() -> None:
    result = run_audit()

    assert result.phase == "17G"
    assert result.baseline_phase == "17F"
    assert result.candidate == "Restricted Choice Supporting Source Audit"
    assert result.classification_before == "SOURCE_PARTIAL"
    assert result.classification_after == "SOURCE_PARTIAL"


def test_phase17g_source_provenance_remains_blocked() -> None:
...
def test_phase17g_protected_sources_are_not_safe_to_stage() -> None:
...
def test_phase17g_vacant_places_contract_evidence_is_present() -> None:
    historical = _recorded()
...
def test_phase17g_restricted_choice_observation_evidence_is_present() -> None:
    historical = _recorded()
...
def test_phase17g_precision_contract_remains_partial() -> None:
    historical = _recorded()
...
def test_phase17g_probability_architecture_is_scaffolding_only() -> None:
    result = run_audit()
...
def test_phase17g_production_remains_blocked() -> None:
    result = run_audit()
...
def test_phase17g_unresolved_dependencies_are_exact() -> None:
    result = run_audit()
...
def test_phase17g_gate_classifications_are_conservative() -> None:
    result = run_audit()
...
def test_phase17g_selects_contract_enrichment_for_phase17h() -> None:
    result = run_audit()

    assert (
        result.phase17h_direction
