...
class PlayEndgameCategoryRepairTests(unittest.TestCase):
    ...
    def test_default_dry_run_selects_only_target_and_writes_nothing(self) -> None:
        ...
        self.assertIn(REVIEWED_ARTICLE, result.output)
        self.assertIn("Files selected      : 1", result.output)
        self.assertIn("Files to update     : 1", result.output)
        self.assertIn("Tag changes         : 0", result.output)
        self.assertIn("Subcategory changes : 0", result.output)
        self.assertNotIn("play/defence/other.md", result.output)
        self.assertNotIn("bidding/other.md", result.output)
