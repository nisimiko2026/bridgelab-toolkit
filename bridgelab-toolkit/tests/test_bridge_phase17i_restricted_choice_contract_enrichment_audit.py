...
from benchmarks.phase17i_restricted_choice_contract_enrichment_audit import (
...
def _recorded():
    validation = validate_historical_report("17I")
    assert validation.is_valid_recorded_output
    assert validation.payload is not None
    return validation.payload


def test_phase17i_candidate_and_classification() -> None:
    result = run_audit()

    assert result.phase == "17I"
    assert result.baseline_phase == "17H"
    assert result.candidate == "Restricted Choice Contract Enrichment Audit"
    assert result.classification_before == "SOURCE_PARTIAL"
    assert result.classification_after == "SOURCE_PARTIAL"


def test_phase17i_source_provenance_remains_blocked() -> None:
...
def test_phase17i_protected_sources_are_not_safe_to_stage() -> None:
...
def test_phase17i_vacant_places_contract_evidence_is_present() -> None:
    historical = _recorded()
...
def test_phase17i_restricted_choice_observation_evidence_is_present() -> None:
    historical = _recorded()
...
def test_phase17i_precision_contract_remains_partial() -> None:
    historical = _recorded()
...
def test_phase17i_probability_architecture_is_scaffolding_only() -> None:
    result = run_audit()
...
def test_phase17i_production_remains_blocked() -> None:
    result = run_audit()
...
def test_phase17i_unresolved_dependencies_are_exact() -> None:
    result = run_audit()
...
def test_phase17i_gate_classifications_are_conservative() -> None:
    result = run_audit()
...
def test_phase17i_selects_contract_enrichment_for_phase17j() -> None:
    result = run_audit()

    assert (
        result.phase17j_direction
        == "RESTRICTED_CHOICE_CONTRACT_ENRICHMENT"
    )


def test_phase17i_contract_gaps_remain_unresolved() -> None:
    result = run_audit()
