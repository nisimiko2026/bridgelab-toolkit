...
from metadata.category_normalization_batch3_3s2 import (
    REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S2,
    apply_category_normalization_batch3_3s2_report,
    build_category_normalization_batch3_3s2_report,
)
...
def article_text(
    ...
    return (
        f"---{newline}title: Entry Management{newline}description: Exact fixture.{newline}"
        ...
        f"status: Draft{newline}unknown: exact{newline}---{newline}# Entry Management{newline}{newline}Body unchanged.{newline}"
    )


class CategoryNormalizationBatch33s2Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def target(self, newline: str = "\n") -> Path:
        relative, (_, tags) = next(
            iter(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S2.items())
        )
    ...
    def invoke(self, *args: str):
        return self.runner.invoke(
            app,
            [
                "repair-category-normalization-batch3-3s2",
                "--root",
                str(self.root),
                *args,
            ],
    ...
    def test_exact_selection_excludes_all_other_planning_files(self) -> None:
        self.assertEqual(len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S2), 1)
        target = self.target()
        _, tags = next(iter(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S2.values()))
        excluded = []
        for name in (
            "timing.md",
            ...
            path = self.root / "play/declarer-play/planning" / name
    ...
    def test_category_only_lf_crlf_exact_tags_and_subcategory(self) -> None:
        for newline in ("\n", "\r\n"):
            with self.subTest(newline=repr(newline)):
                ...
                action = build_category_normalization_batch3_3s2_report(
                    self.root
                ).actions[0]
                ...
                _, tags = next(
                    iter(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S2.values())
                )
    ...
    def test_stale_category_subcategory_and_tag_failures(self) -> None:
        ...
        for old, new, message in (
            ...
            with self.assertRaisesRegex(RuntimeError, message):
                build_category_normalization_batch3_3s2_report(self.root)
    ...
    def test_stale_plan_backup_requirement_and_overwrite_refusal(self) -> None:
        ...
        report = build_category_normalization_batch3_3s2_report(self.root)
    ...
    def test_path_preserving_backup_and_atomic_failure(self) -> None:
        ...
        original = target.read_bytes()
        with patch(
            "metadata.category_normalization_batch3_3s2._atomic_write",
            side_effect=OSError("atomic replacement failed"),
        ):
            result = self.invoke("--apply", "--backup", str(self.backup))
        self.assertNotEqual(result.exit_code, 0)
        self.assertEqual(target.read_bytes(), original)
        relative = next(iter(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S2))
