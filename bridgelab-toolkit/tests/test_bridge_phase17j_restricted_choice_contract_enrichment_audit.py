...
from benchmarks.phase17j_restricted_choice_contract_enrichment_audit import (
...
def test_phase17j_candidate_and_classification() -> None:
    ...
    assert result.phase == "17J"
    assert result.baseline_phase == "17I"
...
def test_phase17j_production_remains_blocked() -> None:
    result = run_audit()
...
def test_phase17j_unresolved_dependencies_are_exact() -> None:
    result = run_audit()
...
def test_phase17j_gate_classifications_are_conservative() -> None:
    result = run_audit()
...
def test_phase17j_selects_contract_enrichment_for_phase17k() -> None:
    result = run_audit()

    assert (
        result.phase17k_direction
        == "RESTRICTED_CHOICE_CONTRACT_ENRICHMENT"
    )


def test_phase17j_contract_gaps_remain_unresolved() -> None:
    result = run_audit()
