...
def article_text(category: str, newline: str = "\n") -> str:
    return (
        f"---{newline}title: Deception{newline}description: Exact fixture bytes.{newline}"
        ...
        f"- deception{newline}systems: []{newline}aliases: []{newline}acronyms: []{newline}"
        f"references: []{newline}last_updated: 2026-08-17{newline}status: Draft{newline}"
        f"unknown_field: exact{newline}---{newline}# Deception{newline}{newline}Body exact.{newline}"
    )


class PlayDeclarerDeceptionRepairTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def targets(self) -> dict[str, Path]:
        return {
            ...
            for index, (relative, category) in enumerate(
                REVIEWED_DECLARER_DECEPTION_CATEGORIES.items()
            )
        }
    ...
    def invoke(self, *args: str):
        return self.runner.invoke(
            app,
            ["repair-play-declarer-deception-categories", "--root", str(self.root), *args],
        )

    def test_dry_run_exact_scope_and_apply_byte_preservation(self) -> None:
        targets = self.targets()
        other = self.write(
            "play/declarer-play/deceptive-play/other.md",
            article_text("techniques/deception"),
        )
        before = {path: path.read_bytes() for path in [*targets.values(), other]}
        dry = self.invoke()
        self.assertEqual(dry.exit_code, 0, dry.output)
        self.assertEqual(dry.output.count("SET CATEGORY |"), 5)
        self.assertIn("Files selected      : 5", dry.output)
        self.assertIn("Tag changes         : 0", dry.output)
        self.assertNotIn("deceptive-play/other.md", dry.output)
        ...
        for relative, path in targets.items():
            ...
            category = REVIEWED_DECLARER_DECEPTION_CATEGORIES[relative]
            ending = b"\r\n" if b"\r\n" in original else b"\n"
    ...
    def test_backup_required_stale_preflight(self) -> None:
        targets = self.targets()
        self.assertNotEqual(self.invoke("--apply").exit_code, 0)
        report = build_play_declarer_deception_category_report(self.root)
        ...
        with self.assertRaisesRegex(RuntimeError, "precondition mismatch"):
            apply_play_declarer_deception_category_report(report, self.root, self.backup)
    ...
    def test_mid_batch_failure_has_all_backups_and_no_false_success(self) -> None:
        ...
        ordered = list(REVIEWED_DECLARER_DECEPTION_CATEGORIES)
    ...
    def test_idempotence_explicit_root_and_help(self) -> None:
        ...
        self.assertEqual(build_play_declarer_deception_category_report(self.root).actions, ())
        second = self.invoke("--apply")
        help_result = self.runner.invoke(
            app, ["repair-play-declarer-deception-categories", "--help"]
        )
        self.assertEqual(second.exit_code, 0, second.output)
        self.assertIn("Files selected      : 5", second.output)
