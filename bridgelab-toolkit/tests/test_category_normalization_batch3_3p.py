...
from metadata.category_normalization_batch3_3p import (
    REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3P,
    apply_category_normalization_batch3_3p_report,
    build_category_normalization_batch3_3p_report,
)
...
def article_text(
    ...
    return (
        f"---{newline}title: Core Convention{newline}description: Exact fixture.{newline}"
        ...
        f"status: Draft{newline}unknown: exact{newline}---{newline}# Core Convention{newline}{newline}Body unchanged.{newline}"
    )


class CategoryNormalizationBatch33pTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def targets(self) -> dict[str, Path]:
        ...
        for index, (relative, (subcategory, tags)) in enumerate(
            sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3P.items())
        ):
    ...
    def invoke(self, *args: str):
        return self.runner.invoke(
            app,
            [
                "repair-category-normalization-batch3-3p",
                "--root",
                str(self.root),
                *args,
            ],
    ...
    def test_exact_selection_excludes_singular_prior_batches_and_unrelated(
        self,
    ) -> None:
        self.assertEqual(len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3P), 16)
        targets = self.targets()
        subcategory, tags = next(
            iter(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3P.values())
        )
        excluded_data = {
            "bidding/conventions/doubles/action-double.md": "Convention",
            "bidding/conventions/competitive/aspro.md": "Conventions",
            "bidding/conventions/responses/stayman.md": "Conventions",
            "bidding/conventions/doubles/unrelated.md": "Conventions",
            "bidding/conventions/opening-bids/unrelated.md": "Conventions",
            "bidding/conventions/slam-conventions/unrelated.md": "Conventions",
        }
        excluded = []
        ...
        self.assertEqual(first.output.count("SET CATEGORY |"), 16)
        self.assertIn("Files selected      : 16", first.output)
    ...
    def test_category_only_lf_crlf_exact_tags_and_subcategories(self) -> None:
        ...
        report = build_category_normalization_batch3_3p_report(self.root)
        self.assertEqual(len(report.actions), 16)
        for action in report.actions:
            ...
            subcategory, tags = REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3P[
                action.article
            ]
            ...
            self.assertIn(f"  - {subcategory.lower()}".encode() + ending, expected)
    ...
    def test_stale_category_subcategory_and_tag_failures(self) -> None:
        ...
        subcategory, tags = REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3P[relative]
        ...
        for old, new, message in (
            ...
            with self.assertRaisesRegex(RuntimeError, message):
                build_category_normalization_batch3_3p_report(self.root)
    ...
    def test_path_preserving_backups_and_atomic_failure(self) -> None:
        ...
        with patch(
            "metadata.category_normalization_batch3_3p._atomic_write",
    ...
    def test_idempotence_explicit_root_apply_and_cli_help(self) -> None:
        ...
        self.assertEqual(
            build_category_normalization_batch3_3p_report(self.root).actions, ()
        )
        ...
        help_result = self.runner.invoke(
            app, ["repair-category-normalization-batch3-3p", "--help"]
        )
