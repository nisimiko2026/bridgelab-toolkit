...
from metadata.play_endgame_category_repair import (
    ...
    apply_play_general_techniques_category_report,
    build_play_general_techniques_category_report,
)
...
def article_text(newline: str = "\n") -> str:
    return (
        f"---{newline}title: Technique{newline}description: Exact fixture bytes.{newline}"
        f"category: techniques/card-play{newline}subcategory: declarer-play{newline}"
        f"difficulty: Intermediate{newline}tags:{newline}- techniques/card-play{newline}"
        f"- technique{newline}systems: []{newline}aliases: []{newline}acronyms: []{newline}"
        f"references: []{newline}last_updated: 2026-08-17{newline}status: Draft{newline}"
        f"unknown_field: exact{newline}---{newline}# Technique{newline}{newline}Body exact.{newline}"
    )


class PlayGeneralTechniquesRepairTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def targets(self) -> dict[str, Path]:
        return {
            ...
            for index, relative in enumerate(REVIEWED_GENERAL_TECHNIQUES_CATEGORIES)
        }

    def invoke(self, *args: str):
        return self.runner.invoke(
            app,
            ["repair-play-general-techniques-categories", "--root", str(self.root), *args],
        )

    def test_dry_run_exact_scope_and_apply_byte_preservation(self) -> None:
        targets = self.targets()
        other = self.write("play/declarer-play/general-techniques/other.md", article_text())
        before = {path: path.read_bytes() for path in [*targets.values(), other]}
        dry = self.invoke()
        self.assertEqual(dry.exit_code, 0, dry.output)
        self.assertEqual(dry.output.count("SET CATEGORY |"), 3)
        self.assertIn("Files selected      : 3", dry.output)
        self.assertIn("Tag changes         : 0", dry.output)
        self.assertNotIn("general-techniques/other.md", dry.output)
        ...
        for relative, path in targets.items():
            ...
            expected = original.replace(
                b"category: techniques/card-play" + ending, b"category: play" + ending, 1
            )
            ...
            self.assertIn(b"- techniques/card-play" + ending, expected)
            self.assertNotIn(b"- play" + ending, expected)

    def test_backup_required_stale_preflight_and_overwrite_refusal(self) -> None:
        targets = self.targets()
        self.assertNotEqual(self.invoke("--apply").exit_code, 0)
        report = build_play_general_techniques_category_report(self.root)
        ...
        with self.assertRaisesRegex(RuntimeError, "precondition mismatch"):
            apply_play_general_techniques_category_report(report, self.root, self.backup)
    ...
    def test_mid_batch_failure_has_all_backups_and_no_false_success(self) -> None:
        ...
        ordered = list(REVIEWED_GENERAL_TECHNIQUES_CATEGORIES)
    ...
    def test_idempotence_explicit_root_and_help(self) -> None:
        ...
        self.assertEqual(build_play_general_techniques_category_report(self.root).actions, ())
        second = self.invoke("--apply")
        help_result = self.runner.invoke(
            app, ["repair-play-general-techniques-categories", "--help"]
        )
