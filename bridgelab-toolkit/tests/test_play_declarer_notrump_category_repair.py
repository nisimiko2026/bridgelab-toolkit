...
from metadata.play_endgame_category_repair import (
    ...
    apply_play_declarer_notrump_category_report,
    build_play_declarer_notrump_category_report,
)
...
def article_text(category: str, newline: str = "\n") -> str:
    return (
        f"---{newline}title: Notrump{newline}description: Exact fixture bytes.{newline}"
        ...
        f"- notrump{newline}systems: []{newline}aliases: []{newline}acronyms: []{newline}"
        f"references: []{newline}last_updated: 2026-08-17{newline}status: Draft{newline}"
        f"unknown_field: exact{newline}---{newline}# Notrump{newline}{newline}Body exact.{newline}"
    )


class PlayDeclarerNotrumpRepairTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def targets(self) -> dict[str, Path]:
        return {
            ...
            for index, (relative, category) in enumerate(
                REVIEWED_DECLARER_NOTRUMP_CATEGORIES.items()
            )
        }
    ...
    def invoke(self, *args: str):
        return self.runner.invoke(
            app,
            ["repair-play-declarer-notrump-categories", "--root", str(self.root), *args],
        )

    def test_dry_run_exact_scope_and_apply_byte_preservation(self) -> None:
        targets = self.targets()
        other = self.write(
            "play/declarer-play/notrump-play/other.md", article_text("card-play/notrump-play")
        )
        ...
        self.assertNotIn("notrump-play/other.md", dry.output)
        ...
        for relative, path in targets.items():
            ...
            category = REVIEWED_DECLARER_NOTRUMP_CATEGORIES[relative]
            ending = b"\r\n" if b"\r\n" in original else b"\n"
    ...
    def test_backup_required_and_stale_preflight(self) -> None:
        ...
        report = build_play_declarer_notrump_category_report(self.root)
        stale = list(targets.values())[3]
        ...
        with self.assertRaisesRegex(RuntimeError, "precondition mismatch"):
            apply_play_declarer_notrump_category_report(report, self.root, self.backup)
    ...
    def test_mid_batch_failure_has_all_backups_and_no_false_success(self) -> None:
        ...
        def fail_fourth(path: Path, content: bytes) -> None:
            ...
            calls += 1
            if calls == 4:
                raise OSError("fourth failed")
            _atomic_write(path, content)

        with patch("metadata.play_endgame_category_repair._atomic_write", side_effect=fail_fourth):
            result = self.invoke("--apply", "--backup", str(self.backup))
        self.assertNotEqual(result.exit_code, 0)
        self.assertNotIn("Files updated", result.output)
        ordered = list(REVIEWED_DECLARER_NOTRUMP_CATEGORIES)
        for relative in ordered[:3]:
    ...
    def test_idempotence_explicit_root_and_help(self) -> None:
        ...
        self.assertEqual(build_play_declarer_notrump_category_report(self.root).actions, ())
        second = self.invoke("--apply")
        help_result = self.runner.invoke(
            app, ["repair-play-declarer-notrump-categories", "--help"]
        )
        self.assertEqual(second.exit_code, 0, second.output)
        self.assertIn("Files selected      : 8", second.output)
