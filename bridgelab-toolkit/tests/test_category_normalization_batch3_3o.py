...
from metadata.category_normalization_batch3_3o import (
    REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3O,
    apply_category_normalization_batch3_3o_report,
    build_category_normalization_batch3_3o_report,
)
...
def article_text(
    ...
    return (
        ...
        f"status: Draft{newline}unknown: exact{newline}---{newline}# Response Convention{newline}{newline}Body unchanged.{newline}"
    )


class CategoryNormalizationBatch33oTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def targets(self) -> dict[str, Path]:
        ...
        for index, (relative, (subcategory, tags)) in enumerate(
            sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3O.items())
        ):
    ...
    def invoke(self, *args: str):
        return self.runner.invoke(
            app,
            [
                "repair-category-normalization-batch3-3o",
                "--root",
                str(self.root),
                *args,
            ],
    ...
    def test_exact_selection_excludes_singular_other_plural_and_transfers(self) -> None:
        self.assertEqual(len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3O), 17)
        targets = self.targets()
        subcategory, tags = next(
            iter(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3O.values())
        )
        excluded_data = {
            "bidding/conventions/responses/inverted-minors.md": "Convention",
            "bidding/conventions/competitive/aspro.md": "Conventions",
            "bidding/conventions/opening-bids/namyats.md": "Conventions",
            "bidding/conventions/slam-conventions/blackwood.md": "Conventions",
            "bidding/conventions/transfers/unrelated-transfer.md": "Conventions",
        }
        excluded = []
    ...
    def test_category_only_lf_crlf_exact_tags_and_subcategories(self) -> None:
        ...
        report = build_category_normalization_batch3_3o_report(self.root)
        ...
        for action in report.actions:
            ...
            subcategory, tags = REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3O[
                action.article
            ]
    ...
    def test_stale_category_subcategory_and_tag_failures(self) -> None:
        ...
        subcategory, tags = REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3O[relative]
        ...
        for old, new, message in (
            ...
            with self.assertRaisesRegex(RuntimeError, message):
                build_category_normalization_batch3_3o_report(self.root)
    ...
    def test_path_preserving_backups_and_atomic_failure(self) -> None:
        ...
        with patch(
            "metadata.category_normalization_batch3_3o._atomic_write",
    ...
    def test_idempotence_explicit_root_apply_and_cli_help(self) -> None:
        ...
        self.assertEqual(
            build_category_normalization_batch3_3o_report(self.root).actions, ()
        )
        ...
        help_result = self.runner.invoke(
            app, ["repair-category-normalization-batch3-3o", "--help"]
        )
