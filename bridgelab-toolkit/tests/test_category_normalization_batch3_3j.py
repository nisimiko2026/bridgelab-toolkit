...
from metadata.category_normalization_batch3_3j import (
    REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3J,
    apply_category_normalization_batch3_3j_report,
    build_category_normalization_batch3_3j_report,
)
...
def article_text(tags: list[str], newline: str = "\n") -> str:
    ...
    return (
        f"---{newline}title: Reviewed Natural Response{newline}"
        ...
        f"---{newline}# Reviewed Natural Response{newline}{newline}Body  unchanged.{newline}"
    )


class CategoryNormalizationBatch33jTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def targets(self) -> dict[str, Path]:
        ...
        for index, (relative, tags) in enumerate(
            sorted(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3J.items())
        ):
    ...
    def invoke(self, *args: str):
        return self.runner.invoke(
            app,
            [
                "repair-category-normalization-batch3-3j",
                "--root",
                str(self.root),
                *args,
            ],
    ...
    def test_exact_selection_excludes_batch_i_indexes_and_dry_run_is_immutable(
        self,
    ) -> None:
        self.assertEqual(len(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3J), 15)
        targets = self.targets()
        tags = next(iter(REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3J.values()))
        excluded_relatives = (
        ...
        self.assertEqual(first.output.count("SET CATEGORY |"), 15)
        self.assertIn("Files selected      : 15", first.output)
        for relative in excluded_relatives:
    ...
    def test_category_only_lf_crlf_exact_tags_and_subcategory(self) -> None:
        ...
        report = build_category_normalization_batch3_3j_report(self.root)
        self.assertEqual(len(report.actions), 15)
        for action in report.actions:
            ...
            tags = REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3J[action.article]
    ...
    def test_stale_category_subcategory_and_tag_failures(self) -> None:
        ...
        for old, new, message in (
            ...
            (b"  - acol\r\n  - blackwood", b"  - blackwood\r\n  - acol", "frozen-tag"),
        ):
            path.write_bytes(original.replace(old, new, 1))
            with self.assertRaisesRegex(RuntimeError, message):
                build_category_normalization_batch3_3j_report(self.root)
    ...
    def test_path_preserving_backups_and_atomic_failure(self) -> None:
        ...
        with patch(
            "metadata.category_normalization_batch3_3j._atomic_write",
    ...
    def test_idempotence_explicit_root_apply_and_cli_help(self) -> None:
        ...
        self.assertEqual(
            build_category_normalization_batch3_3j_report(self.root).actions, ()
        )
        ...
        help_result = self.runner.invoke(
            app, ["repair-category-normalization-batch3-3j", "--help"]
        )
