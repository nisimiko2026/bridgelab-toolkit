...
from metadata.play_endgame_category_repair import (
    ...
    apply_play_declarer_squeezes_category_report,
    build_play_declarer_squeezes_category_report,
)
...
def article_text(category: str, newline: str = "\n") -> str:
    return (
        f"---{newline}title: Squeeze{newline}description: Exact fixture bytes.{newline}"
        ...
        f"- squeeze{newline}systems: []{newline}aliases: []{newline}acronyms: []{newline}"
        f"references: []{newline}last_updated: 2026-08-17{newline}status: Draft{newline}"
        f"unknown_field: exact{newline}---{newline}# Squeeze{newline}{newline}Body exact.{newline}"
    )


class PlayDeclarerSqueezesRepairTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def targets(self) -> dict[str, Path]:
        return {
            ...
            for index, (relative, category) in enumerate(
                REVIEWED_DECLARER_SQUEEZES_CATEGORIES.items()
            )
        }
    ...
    def invoke(self, *args: str):
        return self.runner.invoke(
            app,
            ["repair-play-declarer-squeezes-categories", "--root", str(self.root), *args],
        )

    def test_dry_run_exact_scope_and_apply_byte_preservation(self) -> None:
        targets = self.targets()
        other = self.write(
            "play/declarer-play/squeezes/other.md", article_text("techniques/squeezes")
        )
        ...
        self.assertNotIn("squeezes/other.md", dry.output)
        ...
        for relative, path in targets.items():
            ...
            category = REVIEWED_DECLARER_SQUEEZES_CATEGORIES[relative]
            ending = b"\r\n" if b"\r\n" in original else b"\n"
    ...
    def test_backup_required_and_stale_preflight(self) -> None:
        ...
        report = build_play_declarer_squeezes_category_report(self.root)
        stale = list(targets.values())[4]
        ...
        with self.assertRaisesRegex(RuntimeError, "precondition mismatch"):
            apply_play_declarer_squeezes_category_report(report, self.root, self.backup)
    ...
    def test_mid_batch_failure_has_all_backups_and_no_false_success(self) -> None:
        ...
        ordered = list(REVIEWED_DECLARER_SQUEEZES_CATEGORIES)
    ...
    def test_idempotence_explicit_root_and_help(self) -> None:
        ...
        self.assertEqual(build_play_declarer_squeezes_category_report(self.root).actions, ())
        second = self.invoke("--apply")
        help_result = self.runner.invoke(
            app, ["repair-play-declarer-squeezes-categories", "--help"]
        )
