"""CLI for the reviewed Phase 3A category normalization Batch 3.3c."""
...
def run(root: Path, backup: Path | None, apply: bool) -> None:
    report = build_category_normalization_batch3_3c_report(root)
    for action in report.actions:
        typer.echo(
            f"SET CATEGORY | {action.article} | {OBSERVED_CATEGORY!r} -> "
            f"{PROPOSED_CATEGORY!r} | tags frozen | subcategory='Defenses' frozen"
        )
    typer.echo()
    ...
    apply_category_normalization_batch3_3c_report(report, root, backup)
