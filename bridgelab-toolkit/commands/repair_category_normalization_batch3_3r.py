"""CLI for the reviewed Phase 3A category normalization Batch 3.3r."""

from pathlib import Path
import typer
from metadata.category_normalization_batch3_3r import (
    OBSERVED_CATEGORY,
    PROPOSED_CATEGORY,
    REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3R,
    apply_category_normalization_batch3_3r_report,
    build_category_normalization_batch3_3r_report,
)


def run(root: Path, backup: Path | None, apply: bool) -> None:
    report = build_category_normalization_batch3_3r_report(root)
    for action in report.actions:
        subcategory, _ = REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3R[action.article]
        typer.echo(
    ...
    apply_category_normalization_batch3_3r_report(report, root, backup)
