"""CLI for the reviewed Phase 3A category normalization Batch 3.3s2."""
...
def run(root: Path, backup: Path | None, apply: bool) -> None:
    report = build_category_normalization_batch3_3s2_report(root)
    for action in report.actions:
        subcategory, _ = REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3S2[action.article]
        typer.echo(
    ...
    apply_category_normalization_batch3_3s2_report(report, root, backup)
