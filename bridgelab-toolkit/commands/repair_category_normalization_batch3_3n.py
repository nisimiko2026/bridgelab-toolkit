"""CLI for the reviewed Phase 3A category normalization Batch 3.3n."""

from pathlib import Path
import typer
from metadata.category_normalization_batch3_3n import (
    OBSERVED_CATEGORY,
    PROPOSED_CATEGORY,
    REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3N,
    apply_category_normalization_batch3_3n_report,
    build_category_normalization_batch3_3n_report,
)


def run(root: Path, backup: Path | None, apply: bool) -> None:
    report = build_category_normalization_batch3_3n_report(root)
    for action in report.actions:
        subcategory, _ = REVIEWED_CATEGORY_NORMALIZATION_BATCH3_3N[action.article]
        typer.echo(
    ...
    apply_category_normalization_batch3_3n_report(report, root, backup)
