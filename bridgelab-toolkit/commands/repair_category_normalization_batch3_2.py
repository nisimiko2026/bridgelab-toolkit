"""CLI for the reviewed Phase 3A category normalization Batch 3.2."""
...
def run(root: Path, backup: Path | None, apply: bool) -> None:
    report = build_category_normalization_batch3_2_report(root)
    for action in report.actions:
        typer.echo(
            f"SET CATEGORY | {action.article} | {action.observed!r} -> 'bidding' | "
            "tags frozen"
        )
    typer.echo()
    ...
    apply_category_normalization_batch3_2_report(report, root, backup)
