"""CLI reporting for the reviewed play defence-counting category repair."""
...
def run(root: Path, backup: Path | None, apply: bool) -> None:
    report = build_play_counting_category_report(root)
    for action in report.actions:
        typer.echo(
            ...
            f"present={action.retained_tag_present} | tags frozen"
        )
    ...
    apply_play_counting_category_report(report, root, backup)
