"""CLI for reviewed atomic Phase 3A Batch 3.3af."""
...
def run(root: Path, backup: Path | None, apply: bool) -> None:
    report = build_category_normalization_batch3_3af_report(root)
    for action in report.actions:
        typer.echo(
            f"SET CATEGORY | {action.article} | {OBSERVED_CATEGORY!r} -> "
            f"{PROPOSED_CATEGORY!r} | tags frozen | subcategory={REQUIRED_SUBCATEGORY!r} frozen"
        )
    typer.echo()
    ...
    apply_category_normalization_batch3_3af_report(report, root, backup)
