"""CLI for the reviewed Phase 3A category normalization Batch 3.3u."""
...
def run(root: Path, backup: Path | None, apply: bool) -> None:
    report = build_category_normalization_batch3_3u_report(root)
    for action in report.actions:
        typer.echo(
            f"SET CATEGORY | {action.article} | {OBSERVED_CATEGORY!r} -> {PROPOSED_CATEGORY!r} | tags frozen | subcategory={REQUIRED_SUBCATEGORY!r} frozen"
        )
    typer.echo()
    ...
    apply_category_normalization_batch3_3u_report(report, root, backup)
