"""CLI for the reviewed four-file declarer-coups category batch."""
...
def run(root: Path, backup: Path | None, apply: bool) -> None:
    report = build_play_coups_category_report(root)
    for action in report.actions:
        typer.echo(
            ...
            f"retained-tag={action.retained_tag!r} "
            f"present={action.retained_tag_present} | broad-tag-present="
            f"{action.canonical_tag_present} | tags frozen"
        )
    typer.echo()
    ...
    apply_play_coups_category_report(report, root, backup)
