...
from commands.repair_category_normalization_batch3_3ac import (
    run as repair_category_normalization_batch3_3ac_command,
)
from commands.repair_category_normalization_batch3_3ad import (
    run as repair_category_normalization_batch3_3ad_command,
)
from commands.repair_category_normalization_batch3_3ae import (
    run as repair_category_normalization_batch3_3ae_command,
)
from commands.repair_category_normalization_batch3_3af import (
...
@app.command("repair-category-normalization-batch3-3ac")
def repair_category_normalization_batch3_3ac(
    root: Path = repository_option(),
    backup: Path | None = typer.Option(
        None, "--backup", help="Fresh path-preserving backup directory (required with --apply)."
    ),
    ...
    """Repair Phase 3A category normalization Batch 3.3ac; dry-run by default."""
    repair_category_normalization_batch3_3ac_command(root, backup, apply)


@app.command("repair-category-normalization-batch3-3ad")
def repair_category_normalization_batch3_3ad(
    root: Path = repository_option(),
    backup: Path | None = typer.Option(
        None, "--backup", help="Fresh path-preserving backup directory (required with --apply)."
    ),
    ...
    """Repair Phase 3A category normalization Batch 3.3ad; dry-run by default."""
    repair_category_normalization_batch3_3ad_command(root, backup, apply)


@app.command("repair-category-normalization-batch3-3ae")
def repair_category_normalization_batch3_3ae(
    root: Path = repository_option(),
    backup: Path | None = typer.Option(
        None,
        "--backup",
        help="Fresh path-preserving backup directory (required with --apply).",
    ),
    apply: bool = typer.Option(
        False,
        "--apply",
        help="Apply the reviewed one-file category-line batch.",
    ),
) -> None:
    """Repair Phase 3A category normalization Batch 3.3ae; dry-run by default."""
    repair_category_normalization_batch3_3ae_command(root, backup, apply)


@app.command("repair-category-normalization-batch3-3af")
def repair_category_normalization_batch3_3af(
    root: Path = repository_option(),
    backup: Path | None = typer.Option(
        None, "--backup", help="Fresh path-preserving backup directory (required with --apply)."
    ),
    apply: bool = typer.Option(
        False, "--apply", help="Apply the reviewed atomic three-file category-line batch."
    ),
) -> None:
    """Repair Phase 3A category normalization Batch 3.3af; dry-run by default."""
    repair_category_normalization_batch3_3af_command(root, backup, apply)


@app.command("repair-blue-club-status")
def repair_blue_club_status(
    root: Path = repository_option(),
    backup: Path | None = typer.Option(
        None, "--backup", help="Fresh path-preserving backup directory (required with --apply)."
    ),
    apply: bool = typer.Option(
        False, "--apply", help="Apply the reviewed one-file status-line repair."
    ),
) -> None:
...
@app.command("repair-title-h1-content-repair-batch3")
def repair_title_h1_content_repair_batch3(
    root: Path = repository_option(),
    backup: Path | None = typer.Option(
        None, "--backup", help="Fresh path-preserving backup directory (required with --apply)."
    ),
    apply: bool = typer.Option(
        False, "--apply", help="Apply the reviewed two-file title/H1 content repair."
    ),
) -> None:
