---
title: Placing The Card
description: Explains how to identify the likely location of a specific unseen card through counting and inference.
category: play
subcategory: counting
difficulty: Intermediate
tags: 
  - counting
  - finesse
  - lead
  - opening
  - play
systems: []
aliases: []
acronyms: []
references: 
  - acronyms
  - play/counting/counting-index
  - play/declarer-play/elimination-and-endplays/elimination
  - play/declarer-play/general-techniques/finesses/finesse
  - play/declarer-play/probability/probability-inference
  - play/declarer-play/squeezes/squeezes-index
  - references/references-index
last_updated: 2026-07-27
status: Draft
---


# Placing the Card

## Overview

Placing the card is the process of determining which opponent holds a specific unseen card by combining information from the auction, the play, counting, and logical inference.

## Why Place a Card?

- Improve finesse decisions
- Choose between competing lines
- Plan squeezes
- Execute endplays
- Avoid unnecessary guesses

## Sources of Information

### The Auction

### High-Card Points

### Shape

### Opening Lead

### Defender's Signals

### Played Cards

## Elimination

Remove impossible holdings until only one location remains.

## Examples

### Placing the Queen

### Placing the King

### Placing the Jack

## Common Mistakes

## Related Topics

- counting-points.md
- counting-shape.md
- counting-the-hand.md
- inference.md
